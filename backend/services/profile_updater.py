...
- 理解能力 (comprehension)
- 动手能力 (hands-on)
- 学习初心与动机 (learning-motivation)：提取学生对这门课的期望、动机、目标
...
def _get_onboarding_convs(db: DBSession, student_id: int) -> list[dict]:
    convs = (
        db.query(Conversation)
        .join(ConvSession, Conversation.session_id == ConvSession.id)
        .filter(
...
async def update_student_profile(db: DBSession, student: Student) -> bool:
    ...
    onboarding_convs = _get_onboarding_convs(db, student.id)
    if not convs and not onboarding_convs:
    ...
    onboarding_section = ""
    if onboarding_convs:
        onboarding_text = "\n".join(f"{c['role'].upper()}: {c['content']}" for c in onboarding_convs)
