...
def llm_chat_stream(db: DBSession, system: str, messages: list[dict], document: dict | None = None):
    """Streaming variant of llm_chat. Yields str chunks, then a final dict with token usage."""
    config = get_model_config(db)
    provider = config["provider"]
    model = config["model"]
    ...
    with httpx.stream(
        ...
        for line in resp.iter_lines():
            ...
            try:
                ...
                usage = data.get("usage")
                if usage:
                    yield {"input_tokens": usage.get("prompt_tokens", 0), "output_tokens": usage.get("completion_tokens", 0)}
            except (json.JSONDecodeError, IndexError, KeyError):
                continue
    # If no usage was provided inline, yield a zero-usage marker
