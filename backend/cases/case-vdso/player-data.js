...
<p class="insight">💡 <strong>vvar 页是从内核映射到用户空间的"信息公告板"。内核负责更新，用户态只需看一眼。</strong></p>
<details>
  <summary>📚 深入理解：SeqLock 无锁同步</summary>
  <div class="details-content">
    内核更新 vvar 数据时递增序列号 <code>seq</code>（写入前 +1 变奇数，写入后 +1 变偶数）。<br>
    读者先检查 <code>seq</code> 是否为偶数（奇数说明内核正在写入，必须自旋等待），读取后再检查 <code>seq</code> 是否变化。如果发生变化则重试。这是经典的<strong>读者-写者无锁同步</strong>。
  </div>
</details>`
    },
    {
...
本质上是一个 <code>mmap</code>——内核映射代码页和数据页到每个进程的地址空间。内核通过直接写物理页来更新时间数据，所有进程立刻可见。与 <code>mmap(MAP_SHARED)</code> 原理一脉相承。</p>
