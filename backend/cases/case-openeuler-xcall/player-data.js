...
后面的 <code>do_epoll_wait</code>、<code>copy_from_user</code>、<code>sys_read</code> 是真正干活的内核 I/O 操作，它们是正当的内核态工作。</p>`
    },
    {
      ...
      terminal: {
        prompt: "", command: "",
        output: "标准 ARM64 系统调用入口流程：\\n  1. 硬件切换到 EL1 特权级\\n  2. el0_sync 中保存所有通用寄存器 (X0~X30) → 构建 pt_regs\\n  3. 检查挂起的信号量、调度需求 (TIF_WORK)\\n  4. 解析系统调用号，查表，分发到具体函数\\n  5. 返回时恢复 30+ 寄存器 (ERET)\\n\\n问题：步骤 2 和 5 的寄存器保存/恢复开销\\n  对于 Redis 这种微小请求场景\\n  \"过关费\" > \"办正事的时间\"\\n  → 底噪太大！"
      },
...
<p class="thinking">对于像读写大文件这种极其耗时的操作，这点开销不值一提。但对于 Redis、Nginx 这种处理海量微小请求的程序来说，系统调用本身（保存和恢复几十个寄存器）的开销，甚至超过了系统调用里面实际干的那点活！这叫"底噪"太大。</p>`
    },
    {
      ...
      terminal: {
        prompt: "", command: "",
        output: "xcall 核心设计：VIP 快速绿色通道\\n\\n传统 syscall:  保存 30+ 寄存器 → 检查信号/调度 → 查表分发 → 恢复寄存器\\n  xcall:       保存 3~4 寄存器 →              直通极速处理 → 恢复寄存器\\n\\n关键利用 AAPCS64 ABI 约定：\\n  暂存寄存器 (X0~X7, LR): 调用约定允许破坏\\n  非易失寄存器 (X19~X28): 由被调用者保存\\n  → xcall 只保存底层受损寄存器，连 pt_regs 都不建！\\n\\n四大优化：\\n  ① 指令热替换拦截 (SVC → 魔法跳转指令)\\n  ② 极简现场保存 (仅 3~4 个寄存器)\\n  ③ 抛弃调度/信号检查\\n  ④ 直通目标函数 (绕过查表分发)"
      },
...
xcall 的设计哲学：<b>不干涉内核的实际工作，只精准砍掉"进出城门"的无用开销</b>。不改一行用户代码，用操作系统底层架构创新带来的顶级性能红利。</p>`
    ...
    {
      title: "🎓 进阶：为什么不全用 xcall？",
      terminal: {
        prompt: "", command: "",
        output: "xcall 的三大取舍：\\n\\n1. 丧失进程调度能力\\n   传统 syscall 返回前检查时间片 → 可能被挂起\\n   xcall 没有 pt_regs → 绝对不能被挂起\\n   → 只适用于绝不阻塞的高频调用\\n\\n2. 丧失信号处理时机\\n   Ctrl+C (SIGINT) 通常在 syscall 返回时派发\\n   xcall 砍掉了信号检查逻辑\\n\\n3. 内核追踪 (strace) 失效\\n   strace 利用 ptrace 在 syscall 入口/出口打桩\\n   xcall 快速路径绕过追踪探测点\\n\\n结论：xcall = 白名单旁路加速插件\\n  仅供少量极高频、极低耗时的操作使用"
      },
