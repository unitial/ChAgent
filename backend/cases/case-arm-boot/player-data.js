...
<p>PC 处于 <code>0xffff...</code> 高位地址 = MMU 已开启 = 内核早已进入 C 语言执行阶段。只是串口看不到任何输出。</p>`
    },
    {
      ...
      terminal: {
        prompt: "", command: "",
        output: "ARM64 启动链：\\n  ① ROM 固件 (BL1) [EL3]            ✅ 通过\\n  ② 可信固件 (BL2) [EL3]            ✅ 通过 (DRAM 初始化成功)\\n  ③ EL3 Runtime [EL3]               ✅ 通过 (降级到 EL2)\\n  ④ U-Boot [EL2/EL1]                ✅ 通过 (加载了内核和DTB)\\n  ⑤ Linux 内核 [EL1]                ← CPU 在这里，MMU 开了\\n     但串口没输出！                  ← 内核\"说不出话来\"\\n  ⑥ 用户空间                        ← 还没到这里\\n\\n关键线索：printk() 核心落点 = log_buf (内核环形日志缓冲区)\\n即使串口控制台失败，内存中的日志仍保留关键线索！"
      },
...
<p>一个是 16550 驱动配 <code>ttyS0</code>，一个是 PL011 驱动配 <code>ttyAMA0</code>。<strong>设备树就是这样把"通用内核"和"特定硬件"粘合在一起的。</strong></p>`
    },
    {
      ...
      terminal: {
        prompt: "", command: "",
        output: "ARM64 Linux 内核入口约定：\\n  x0 = DTB 的物理地址    ← \"硬件说明书\"\\n  x1~x3 = 0 (reserved)\\n  PC = 内核 Image 的加载地址\\n  当前特权级 = EL2（推荐）或 EL1\\n  MMU = 关闭, D-cache = 关闭\\n\\nx86 用 BIOS/ACPI 告诉内核硬件配置\\nARM64 用设备树 (DTB) — 一个二进制数据结构\\nU-Boot 将 DTB 加载到内存，通过 x0 寄存器传给内核"
      },
...
<p>这就是 ARM64 上"从上电到内核"的完整故事。</p>`
    },
    {
      ...
      terminal: {
        prompt: "", command: "",
        output: "核心知识点：\\n  1. \"同一个内核\" ≠ \"同一份配置\"\\n     ARM 碎片化 → 每块板子的设备地址/中断号/时钟频率都不同\\n     内核通用, 设备树板级, 搞混了就是 panic\\n  2. ARM64 vs x86 启动：截然不同的哲学\\n     x86: 16位爬到64位, BIOS/UEFI/ACPI 屏蔽硬件差异\\n     ARM64: 一上电就是64位, 硬件差异暴露给软件(DTB)\\n  3. 异常级别是嵌套的保护环\\n     EL3 > EL2 > EL1 > EL0\\n     JTAG 是串口无输出时的终极调试手段\\n  4. 一个 .dtb 错误就能让整条启动链断裂\\n     固件→U-Boot→内核→设备树→驱动→用户空间"
      },
